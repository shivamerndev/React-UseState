const Header = ({ render, setToggle, toggle, setTasks }) => {
    ...
    return <nav id={toggle ? "bg-theme-light" : 'bg-theme'} className='flex w-full justify-between select-none items-center py-4 rounded-2xl px-8'>
        ...
        <h1 onClick={() => setToggle(!toggle)} className='text-2xl cursor-pointer  '>{toggle ? "🌙" : "🌤️"} <span className='text-base cursor-none'>Render : {render}</span></h1>
    </nav>
